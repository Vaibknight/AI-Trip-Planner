 ...
 * Align patterns with `Backend/src/utils/placeNormalization.js`.
 */

const LEADING_ACTIVITY_PATTERNS: RegExp[] = [
  /^arrive\s+(at\s+)?/i,
  /^arrival\s+(at\s+)?/i,
  /^depart\s+(for\s+)?/i,
  /^departure\s+(for\s+)?/i,
...
function isVagueGeocodeSegment(seg: string): boolean {
...
/** Strip activity phrasing; suitable for map geocode query + dedupe keys. */
export function normalizePlaceForGeocode(text: string): string {
...
/** Do not send to geocoder — generic activity / food blurbs with no POI. */
export function shouldSkipGeocodeQuery(cleaned: string): boolean {
